...
def energy_estimation(Jx,Jy,Jz):
    ...
    for i,phi in enumerate(Nt):
        ...
        for circ,obs,res, singleobs,singleres in zip(circuits, observables,results, single_observables, results_single):
            ...
            for spauli, sr in zip(singleobs, singleres):
                print("For pauli ", spauli, "coef: ", spauli.coeffs[0].real)
    ...
    return energy,proy_Sz1,proy_Sz2,proy_Sz3
...
def exact(Jx,Jy,Jz):
    ...
    Szop1 = SparsePauliOp.from_list([("ZII",0.5)])
    Szop2 = SparsePauliOp.from_list([("IZI",0.5)])
    Szop3 = SparsePauliOp.from_list([("IIZ",0.5)])
    # energy estimation
    energy = [];Sz1=[];Sz2=[];Sz3=[]
    for t in phi_vals_:
        U = expm(-1j * Hamil * t)
        ...
        Sz1.append(np.real(exp_val(Szop1.to_matrix(),evolved_state)))
        Sz2.append(np.real(exp_val(Szop2.to_matrix(),evolved_state)))
        Sz3.append(np.real(exp_val(Szop3.to_matrix(),evolved_state)))


    return energy, Sz1,Sz2,Sz3
...
plt.show()
plt.savefig("./results/energy-conservation-three-fake.png", dpi=400)
...
plt.plot(Nt, JxJy[3],"o",alpha=0.7, label=r"$\langle IIS^z\rangle/\hbar$, $J_x=J_y\neq J_z$")
...
plt.plot(Nt, JxJyJz[2],"o",alpha=0.7, label=r"$\langle IS^zI\rangle/\hbar$, $J_x=J_y=J_z$")
...
plt.plot(Nt, different[3],"o",alpha=0.7, label=r"$\langle IIS^z\rangle/\hbar$, $J_x\neq J_y\neq J_z$")
...
plt.show()
plt.savefig("./results/symetries-three-fake.png", dpi=400)